...
```
##### Collectively Breast Invasive Carcinoma's had the most metastatic tumor samples while Colorectum Adenocarcinoma's had the most primary tumor samples and most tumor samples overall. Skin Cutaneous Melenoma had the greastest ratio of metastatic to primary tumor samples. Primary cancers which did not have metastatic tumor samples include: Lung Squamous Cell Carcinoma, Sarcoma, Stomach Adenocarcinoma, Testicular Germ Cell Tumors, and 	Uveal Melanoma. Esophageal Carcinoma was the only primary cancer which had metastatic tumor samples but no primary tumor samples (excluding metastatic tumor samples of Cancer of Unknown Primary)
...
cs1$Data <- "TCGA (RNAseq)"
cs2$`Normal Tissue` <- 0
cs2$Data <- "non-TCGA (microarray)"
...
cs_combined <- cs_combined %>%
  mutate(m_TCGA_label = `Metastatic Tumor`[grepl("TCGA \\(RNAseq\\)", Data)],
         m_nonTCGA_label = ifelse(any(grepl("non-TCGA \\(microarray\\)", Data)), `Metastatic Tumor`[grepl("non-TCGA \\(microarray\\)", Data)], NA), 
         p_TCGA_label = `Primary Tumor`[grepl("TCGA \\(RNAseq\\)", Data)],
         p_nonTCGA_label = ifelse(any(grepl("non-TCGA \\(microarray\\)", Data)), `Primary Tumor`[grepl("non-TCGA \\(microarray\\)", Data)], NA))
...
#remove duplicate abbreviations then used label columns which effectively take data from TCGA and nonTCGA data sets
cs_combined2 <- cs_combined[!duplicated(cs_combined$abbreviation),]

#make and add bar labels
for(i in 1:nrow(cs_combined2)){
  mgraph <- mgraph +
    annotate("text", x = -10, y = cs_combined2$abbreviation[i], label = cs_combined2$m_TCGA_label[i], color = "#45caff", hjust = 1) +
    annotate("text", x = cs_combined2$m_total[i], y = cs_combined2$abbreviation[i], label = cs_combined2$m_nonTCGA_label[i], color = "#ff1b6b", hjust = -.2)
}
```

```{r, echo=FALSE}
# Primary tumor graph showing number of different TCGA primary tumors in microarray and RNAseq datasets
...
for(i in 1:nrow(cs_combined2)){
  pgraph <- pgraph +
    annotate("text", x = cs_combined2$p_total[i], y = cs_combined2$abbreviation[i],
             label = cs_combined2$p_nonTCGA_label[i], color = "#ff1b6b", hjust = 1.1) +
    annotate("text", x = -10, y = cs_combined2$abbreviation[i],
             label = cs_combined2$p_TCGA_label[i], color = "#45caff", hjust = 0)
}
...
## Metastatic Tumors and Primary Disease in non-TCGA and TCGA Datasets 
```{r, echo=FALSE}
mgraph <- mgraph + 
  annotate("text", x = -150, y = min(cs_combined2$abbreviation), label = " ", hjust = 0)

pgraph <- pgraph + 
  annotate("text", x = -250, y = min(cs_combined2$abbreviation), label = " ", hjust = 0) +
...
combined_graph <- combined_graph +
  ...
  theme(
    plot.title = element_text(hjust = -.83, size = 11, face = "bold")
  )
combined_graph
```
...
met_dis1 <- clin1
met_dis1$Data <- "non-TCGA (microarray)"
met_dis2 <- clin2
met_dis2$Data <- "TCGA (RNAseq)"
